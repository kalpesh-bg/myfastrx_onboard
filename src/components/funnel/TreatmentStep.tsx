...
const TreatmentStep = ({
  ...
  return (
    <div className="w-full pb-44 md:pb-28">
      ...
      <p className="text-center text-muted-foreground mb-1 font-medium md:text-[16px] text-sm max-w-xl mx-auto px-2">
        Choose the plan that fits your goals.
      </p>
      <p className="text-center text-primary mb-6 font-medium md:text-[15px] text-sm max-w-xl mx-auto px-2">
        Provider review, medication and shipping included.
      </p>

      {/* Medication selector */}
      <div className="bg-white rounded-lg max-w-5xl mx-auto p-4 md:p-6 mb-4 md:mb-6 border border-border shadow-sm">
          ...
          {(['semaglutide', 'tirzepatide'] as Treatment[]).map((t) => {
            ...
            return (
              <button
                ...
                className={`relative bg-white rounded-md p-4 border-2 text-center transition-all ${
              ...
              >
                ...
                <img src={img} alt={t} className="h-20 md:h-28 mx-auto object-contain mb-2" />
                <div className="font-bold text-base md:text-lg capitalize leading-tight mb-2">{t}</div>
                <span className={`inline-block text-[11px] md:text-xs px-2.5 py-1 rounded-full font-medium ${tag.cls}`}>
                  {tag.label}
                </span>
              </button>
            );
      ...
      {/* Plan selection */}
      <div className="bg-white rounded-lg max-w-5xl mx-auto p-4 md:p-6 mb-4 border border-border shadow-sm">
        <h3 className="font-bold text-lg md:text-xl mb-4 border-l-4 border-primary pl-3">Choose Your Plan</h3>
            ...
            const planBadge =
              plan.badge === 'popular'
                ...
                : plan.badge === 'best'
                  ...
                  : isFirstMonthDeal
                    ? { text: 'MY50 FIRST-MONTH OFFER', cls: 'bg-amber-100 text-amber-700' }
                    : null;
                ...
                <div className="flex items-end justify-between gap-3 pl-7">
                  <div className="min-h-11 flex flex-col justify-end">
                    <div className="flex items-baseline gap-1">
                      <span className="text-2xl font-bold text-foreground">{fmt(plan.monthly)}</span>
                  ...
                  <div className="bg-[#e1f8e8] text-green-700 border border-green-300 text-xs font-bold px-2.5 py-1.5 rounded-md shrink-0">
      ...
      {/* Dosage note */}
      <div className="max-w-5xl mx-auto flex items-start gap-2 text-xs md:text-sm text-muted-foreground mb-6 px-1">
        ...
        <div className="max-w-3xl mx-auto px-4 py-3 md:py-4">
          <div className="flex items-center justify-center gap-0 mb-3">
            <div className="flex-1 text-center pr-4">
              <div className="text-[11px] text-muted-foreground uppercase tracking-wide mb-0.5">Selected Plan</div>
              <div className="font-bold text-sm md:text-base">{selectedPlan.label}</div>
            </div>
            <div className="w-px h-10 bg-border" />
            <div className="flex-1 text-center pl-4">
              <div className="text-[11px] text-muted-foreground uppercase tracking-wide mb-0.5">Due Today</div>
              <div className="font-bold text-primary text-lg md:text-xl">{fmt(dueToday)}</div>
            </div>
          </div>
          <button
